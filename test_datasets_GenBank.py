...
logger.info("Checking if dfast_file_downloader.py exists")
if os.path.exists("dfast_file_downloader.py "):
    logger.info("dfast_file_downloader.py exists, no need to download it again. skipping...")
else:
    logger.info("dfast_file_downloader.py does not exist, downloading it...")
    cmd = ["curl","-o","dfast_file_downloader.py","https://raw.githubusercontent.com/nigyta/dfast_core/master/scripts/dfast_file_downloader.py"]
    run_command(cmd)

# Specify the directory containing our genomes.
genome_directory = "/Users/mohamed/Desktop/dfast_qc/dqc_reference/genomes"
#genome_directory = "/home/melmanzalawi/dfast_qc/dqc_reference/genomes" # HPC
...
df_uniq_genome_dummy = df_uniq_genome.iloc[1:5]
df_uniq_genome_dummy.to_csv("genbank_test_data/genbank_prok_1_per_species_dummy.tsv", columns=["assembly_accession"],  header=True, index=False, sep="\t")
logger.info("Done")
